...
{
    /**
     * @var array<string, array{id: string, name: string, nameDe: string, nameZh:string, availableEntities: array<string, string|null>}>|null
     */
    ...
    private function createLocale(Connection $connection): void
    {
        ...
        foreach ($localeData as $locale) {
            if (\in_array($locale['locale'], ['en-GB', 'de-DE', 'zh-CN'], true)) {
                continue;
            }
    ...
    private function createCountry(Connection $connection): void
    {
        ...
        $connection->insert('country_translation', $languageDE($cnId, 'China'));
    ...
    private function createCurrency(Connection $connection): void
    {
        ...
        $connection->insert('currency', ['id' => $EUR, 'iso_code' => 'EUR', 'factor' => 0.1288, 'symbol' => '€', 'position' => 1, 'decimal_precision' => 2, 'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT)]);
        ...
        $connection->insert('currency', ['id' => $USD, 'iso_code' => 'USD', 'factor' => 0.1378, 'symbol' => '$', 'position' => 1, 'decimal_precision' => 2, 'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT)]);
        ...
        $connection->insert('currency', ['id' => $GBP, 'iso_code' => 'GBP', 'factor' => 0.1086, 'symbol' => '£', 'position' => 1, 'decimal_precision' => 2, 'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT)]);
    ...
    private function createPaymentMethod(Connection $connection): void
    {
        ...
        $connection->insert('payment_method_translation', ['payment_method_id' => $debit, 'language_id' => $languageZH, 'name' => '直接扣款', 'description' => '', 'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT)]);
        ...
        $connection->insert('payment_method_translation', ['payment_method_id' => $invoice, 'language_id' => $languageEN, 'name' => 'Invoice', 'description' => 'Payment by invoice. Haoke provides automatic invoicing for all customers on orders after the first, in order to avoid defaults on payment.', 'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT)]);
        $connection->insert('payment_method_translation', ['payment_method_id' => $invoice, 'language_id' => $languageDE, 'name' => 'Rechnung', 'description' => 'Sie zahlen einfach und bequem auf Rechnung. Haoke bietet z.B. auch die Möglichkeit, Rechnung automatisiert erst ab der 2. Bestellung für Kunden zur Verfügung zu stellen, um Zahlungsausfälle zu vermeiden.', 'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT)]);
        $connection->insert('payment_method_translation', ['payment_method_id' => $invoice, 'language_id' => $languageZH, 'name' => '发票', 'description' => '通过发票支付。好客平台为所有客户在第一次订单后的所有订单提供自动开具发票，以避免付款违约.', 'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT)]);
        ...
        $connection->insert('payment_method_translation', ['payment_method_id' => $pre, 'language_id' => $languageZH, 'name' => '提前付款', 'description' => '提前付款，然后再下单.', 'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT)]);
    }
    ...
    private function createShippingMethod(Connection $connection): void
    {
        ...
        $connection->insert('shipping_method_translation', ['shipping_method_id' => $standard, 'language_id' => $languageZH, 'name' => '普通', 'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT)]);
        ...
        $connection->insert('shipping_method_translation', ['shipping_method_id' => $express, 'language_id' => $languageZH, 'name' => '快递', 'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT)]);
    ...
    private function createSalesChannelTypes(Connection $connection): void
    {
        ...
        $connection->insert('sales_channel_type_translation', ['sales_channel_type_id' => $storefront, 'language_id' => $languageZH, 'name' => 'Storefront', 'manufacturer' => 'shopware AG', 'description' => 'Sales channel mit HTML storefront', 'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT)]);
        ...
        $connection->insert('sales_channel_type_translation', ['sales_channel_type_id' => $storefrontApi, 'language_id' => $languageZH, 'name' => 'Headless', 'manufacturer' => 'shopware AG', 'description' => 'API only sales channel', 'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT)]);
    }
    ...
    private function createProductManufacturer(Connection $connection): void
    {
        ...
        $connection->insert('product_manufacturer_translation', ['product_manufacturer_id' => $id, 'product_manufacturer_version_id' => $versionId, 'language_id' => $languageEN, 'name' => 'Haoke AG', 'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT)]);
        $connection->insert('product_manufacturer_translation', ['product_manufacturer_id' => $id, 'product_manufacturer_version_id' => $versionId, 'language_id' => $languageDE, 'name' => 'Haoke AG', 'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT)]);
        $connection->insert('product_manufacturer_translation', ['product_manufacturer_id' => $id, 'product_manufacturer_version_id' => $versionId, 'language_id' => $languageZH, 'name' => 'Haoke AG', 'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT)]);
    }
    ...
    private function createDefaultMediaFolders(Connection $connection): void
    {
        ...
        $queue->addInsert('media_default_folder', ['id' => Uuid::randomBytes(), 'association_fields' => '["avatarUser"]', 'entity' => 'customer', 'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT)]);
    ...
    private function createDocumentTypes(Connection $connection): void
    {
        ...
        $connection->insert('document_type_translation', ['document_type_id' => $invoiceId, 'language_id' => Uuid::fromHexToBytes($this->getZhCnLanguageId()), 'name' => 'Invoice', 'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT)]);
        ...
        $connection->insert('document_type_translation', ['document_type_id' => $deliveryNoteId, 'language_id' => Uuid::fromHexToBytes($this->getZhCnLanguageId()), 'name' => 'Delivery note', 'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT)]);
        ...
        $connection->insert('document_type_translation', ['document_type_id' => $creditNoteId, 'language_id' => Uuid::fromHexToBytes($this->getZhCnLanguageId()), 'name' => 'Credit note', 'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT)]);
    ...
    /**
     * @return array<string, array{id: string, name: string, nameDe: string, nameZh:string, availableEntities: array<string, string|null>}>
     */
    private function getMailTypeMapping(): array
    {
        return $this->mailTypes ?? $this->mailTypes = [
            ...
            'newsletterRegister' => [
                ...
                'nameZh' => 'Newsletter registration',
                'availableEntities' => ['newsletterRecipient' => 'newsletter_recipient', 'salesChannel' => 'sales_channel'],
            ],
            ...
            MailTemplateTypes::MAILTYPE_CUSTOMER_GROUP_CHANGE_ACCEPT => [
                ...
                'nameZh' => 'Customer group change accepted',
                'availableEntities' => ['customer' => 'customer', 'salesChannel' => 'sales_channel'],
            ],
            MailTemplateTypes::MAILTYPE_CUSTOMER_GROUP_CHANGE_REJECT => [
                ...
                'nameZh' => 'Customer group change rejected',
                'availableEntities' => ['customer' => 'customer', 'salesChannel' => 'sales_channel'],
            ],
            MailTemplateTypes::MAILTYPE_PASSWORD_CHANGE => [
                ...
                'nameZh' => 'Password change request',
                'availableEntities' => [
                    'customer' => 'customer',
            ...
            'state_enter.order_delivery.state.shipped_partially' => [
                ...
                'nameZh' => 'Enter order state: Shipped (partially)',
                'availableEntities' => [
                    'order' => 'order',
                    'previousState' => 'state_machine_state',
                    'newState' => 'state_machine_state',
            ...
            'state_enter.order_delivery.state.returned' => [
                ...
                'nameZh' => 'Enter order state: Returned',
                'availableEntities' => [
                    'order' => 'order',
                    'previousState' => 'state_machine_state',
                    'newState' => 'state_machine_state',
            ...
            'state_enter.order_delivery.state.shipped' => [
                ...
                'nameZh' => 'Enter order state: Shipped',
                'availableEntities' => [
                    'order' => 'order',
                    'previousState' => 'state_machine_state',
                    'newState' => 'state_machine_state',
            ...
            'state_enter.order_delivery.state.cancelled' => [
                ...
                'nameZh' => 'Enter order state: Cancelled',
                'availableEntities' => [
                    'order' => 'order',
                    'previousState' => 'state_machine_state',
                    'newState' => 'state_machine_state',
            ...
            'state_enter.order_transaction.state.reminded' => [
                ...
                'nameZh' => 'Enter payment state: Reminded',
                'availableEntities' => [
                    'order' => 'order',
                    'previousState' => 'state_machine_state',
                    'newState' => 'state_machine_state',
            ...
            'state_enter.order_transaction.state.refunded_partially' => [
                ...
                'nameZh' => 'Enter payment state: Refunded (partially)',
                'availableEntities' => [
                    'order' => 'order',
                    'previousState' => 'state_machine_state',
                    'newState' => 'state_machine_state',
            ...
            'state_enter.order_transaction.state.cancelled' => [
                ...
                'nameZh' => 'Enter payment state: Cancelled',
                'availableEntities' => [
                    'order' => 'order',
                    'previousState' => 'state_machine_state',
                    'newState' => 'state_machine_state',
            ...
            'state_enter.order_transaction.state.paid' => [
                ...
                'nameZh' => 'Enter payment state: Paid',
                'availableEntities' => [
                    'order' => 'order',
                    'previousState' => 'state_machine_state',
                    'newState' => 'state_machine_state',
            ...
            'state_enter.order_transaction.state.refunded' => [
                ...
                'nameZh' => 'Enter payment state: Refunded',
                'availableEntities' => [
                    'order' => 'order',
                    'previousState' => 'state_machine_state',
                    'newState' => 'state_machine_state',
            ...
            'state_enter.order_transaction.state.paid_partially' => [
                ...
                'nameZh' => 'Enter payment state: Paid (partially)',
                'availableEntities' => [
                    'order' => 'order',
                    'previousState' => 'state_machine_state',
                    'newState' => 'state_machine_state',
            ...
            'state_enter.order_transaction.state.open' => [
                ...
                'nameZh' => 'Enter payment state: Open',
                'availableEntities' => [
                    'order' => 'order',
                    'previousState' => 'state_machine_state',
                    'newState' => 'state_machine_state',
            ...
            'state_enter.order.state.in_progress' => [
                ...
                'nameZh' => 'Enter order state: In progress',
                'availableEntities' => [
                    'order' => 'order',
                    'previousState' => 'state_machine_state',
                    'newState' => 'state_machine_state',
