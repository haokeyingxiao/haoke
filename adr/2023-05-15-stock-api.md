...
The stock handling in Haoke 6 is currently not very flexible and does not support many common use cases. 
...
In Haoke version 6.6 or if the `STOCK_HANDLING` feature flag is enabled:
...
We update the various locations in Haoke where stock is loaded and augment the product with any stock information that is loaded from the stock storage.
